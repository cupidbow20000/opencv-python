...
def process_frame(frame_number, frame, bg_subtractor, car_counter):
    ...
    cv2.line(processed, (car_counter.divider, 0), (car_counter.divider, frame.shape[0]), DIVIDER_COLOUR, 1)

    # Remove the background
    fg_mask = bg_subtractor.apply(frame, None, 0.01)
    save_frame(IMAGE_DIR + "/fg_mask_%04d.png"
...
def main():
    ...
    default_bg = cv2.imread(IMAGE_FILENAME_FORMAT % 0)
    ...
    while True:
        ...
        if car_counter is None:
            ...
            # log.debug("shape = %d, divide = %d", frame.shape[:2], frame.shape[0] / 2)
            car_counter = VehicleCounter(frame.shape[:2], frame.shape[1] // 2 + 30)
